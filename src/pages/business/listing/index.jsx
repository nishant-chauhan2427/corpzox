...
const handleScroll = (e) => {
  ...
  console.log(bottom,!businessLoading , business?.list?.length,business.totalPage)
  if (bottom && !businessLoading && business?.list?.length<business.totalPage) {
    dispatch(setBusinessPage(business.page + 1)); // Load next page
  }
};
  return (
    <div className="flex flex-col pt-10 overflow-y-auto" onScroll={handleScroll}>
      ...
      {business?.list.length > 0 ? (
        ...
        {!businessLoading&&business.list.length<business.totalPage&&<div className="mt-10 flex justify-center">
                <Button primary={true} onClick={()=>dispatch(setBusinessPage(business.page + 1))}>Load More </Button>
        </div>}
        </>
      ) : (
